...
public:
    Graph(int cols, int rows) : COLUMNS(cols), ROWS(rows) {
        ...
        if (!exitTexture.loadFromFile("D:/DSA/TheMazeRunner/Graphics/treasure.png")) {
            cerr << "Failed to load exit texture" << endl;
        }
        if (!visitedTexture.loadFromFile("D:/DSA/TheMazeRunner/Graphics/idle/body/tile005.png")) {
    ...
    void addNeighborsToFrontier(Node* node, Node* frontier[], int& frontierSize) 
        //frontier  is a set of unvisited nodes
    {
...
int main() {
    ...
    Vector2f offset((vm.width - COLUMNS * CELL_SIZE) / 2.0f, (vm.height - ROWS * CELL_SIZE) / 2.0f); // grid in centre
    ...
    generating_maze_sound_buffer.loadFromFile("D:/DSA/TheMazeRunner/Sounds/mazegenerating.wav.wav");
    ...
    timeout_buffer.loadFromFile("D:/DSA/TheMazeRunner/Sounds/gameOver.wav.wav");
    ...
    howToPlayText.setPosition(300, 300);
    ...
    movesLeftText.setPosition(300, 250); // Top-left corner
    ...
    timeBar.setPosition((1920 - timeBarStartWidth) / 2, 150);
